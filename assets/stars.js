(function(){
  ...
  function tracedVisual(event){var state=event.state||[],simStars=[],simXs=[];for(var i=0;i<state.length;i++){if(state[i]===2&&cells[i]!==2)simStars.push(i);else if(state[i]===1&&cells[i]===0)simXs.push(i)}return{area:(event.area||[]).slice(),targets:(event.targets||event.assigned||event.cells||[]).slice(),problemTiles:event.kind==='conflict'?(event.targets||event.cells||[]).slice():[],simStars:simStars,simXs:simXs}}
  function traceStep(event,prefix){
    ...
    return{title:prefix+title,text:text,visual:tracedVisual(event)}
  }
  function appendTraceSteps(steps,events,prefix,max){
    max=max||28;if(events.length<=max){events.forEach(function(event){steps.push(traceStep(event,prefix))});return}var first=Math.ceil(max*.65),last=max-first;events.slice(0,first).forEach(function(event){steps.push(traceStep(event,prefix))});var bridge=events[events.length-last-1],skipped=events.length-max;steps.push({title:(prefix?prefix+': ':'')+'Continue the repeated deductions',text:skipped+' additional quota and no-touch deductions repeat the same rules. The board shows their combined result.',visual:tracedVisual(bridge)});events.slice(-last).forEach(function(event){steps.push(traceStep(event,prefix))})
  }
  function appendContradictionExplanation(steps,analysis){
    appendTraceSteps(steps,analysis.root&&analysis.root.events||[],'Forced consequence',30);if(analysis.direct)return;
    if(!analysis.contradiction||analysis.unitIndex==null){steps.push({title:'Complete the exhaustive check',text:'Every legal continuation was checked. Each one eventually exceeds a two-star quota, leaves a unit unable to reach two stars, or makes two stars touch.',visual:{area:[],targets:[],problemTiles:[]}});return}
    var unit=context.units[analysis.unitIndex],combos=analysis.branches.map(function(branch){return squareList(branch.chosen,5)});steps.push({title:'List every remaining option',text:unitLabel(analysis.unitIndex,analysis.open[0])+' has '+analysis.allBranches+' legal placement'+(analysis.allBranches===1?'':'s')+' left'+(combos.length?': '+combos.join('; '):'')+'. Each option must be tested.',visual:{area:unit,targets:analysis.open,simStars:[],simXs:[]}});
    analysis.branches.forEach(function(branch,index){steps.push({title:'Option '+(index+1)+' of '+analysis.allBranches,text:'Test stars at '+squareList(branch.chosen)+' and X marks at '+squareList(branch.rejected)+'. Now follow this option until its exact rule failure.',visual:tracedVisual({state:branch.state,area:unit,targets:analysis.open})});appendTraceSteps(steps,branch.path,'Option '+(index+1),18)});
    if(analysis.truncated)steps.push({title:'Check the remaining options',text:'The remaining '+(analysis.allBranches-analysis.branches.length)+' placement'+(analysis.allBranches-analysis.branches.length===1?' also fails':'s also fail')+' under the same exhaustive check.',visual:{area:unit,targets:analysis.open,problemTiles:analysis.open}});
    steps.push({title:'All placements fail',text:'These options exhaust every legal way to place the missing stars in '+unitLabel(analysis.unitIndex,analysis.open[0])+'. Because every option reaches a visible rule violation, the original assumption is impossible.',visual:{area:unit,targets:analysis.open,problemTiles:analysis.open}})
  }
  function problemHint(){
    var wrong=[];for(var i=0;i<n*n;i++)if(cells[i]&&cells[i]!==solution[i])wrong.push(i);if(!wrong.length)return null;var target=wrong[0],wasStar=cells[target]===2,analysis=core.analyzeContradiction(regions,n,cells.slice(),140000,10),steps=[{title:'Review this mark',text:'Start at '+squareName(target)+'. It is currently marked as '+(wasStar?'a star':'an X')+'. Keep it temporarily and follow everything it forces.',visual:{area:[target],targets:[target]}}];appendContradictionExplanation(steps,analysis);steps.push({title:'Correct the original mark',text:solution[target]===2?'The contradiction began with the X at '+squareName(target)+'. Replace it with a star.':'The contradiction began with the star at '+squareName(target)+'. Replace it with an X.',visual:{area:[target],targets:[target],hiddenMarks:[target],simStars:solution[target]===2?[target]:[],simXs:solution[target]===1?[target]:[]}});return{target:target,value:solution[target],summary:'Follow the mark’s consequences to the exact rule that breaks.',steps:steps}
  }
  function fallbackHint(){
    var baseState=effectiveState(),open=[];for(var i=0;i<n*n;i++)if(baseState[i]===0)open.push(i);open=unSeen(open);if(!open.length)return null;var target=open[0],value=solution[target],state=null,analysis=null;
    for(var candidate=0;candidate<Math.min(open.length,60);candidate++){var testTarget=open[candidate],testValue=solution[testTarget],testState=baseState.slice();testState[testTarget]=testValue===2?1:2;var traced=core.traceContradiction(regions,n,testState,160);if(traced.contradiction){target=testTarget;value=testValue;state=testState;analysis={contradiction:true,direct:true,root:traced,branches:[],allBranches:0};break}}
    if(!state){state=baseState.slice();state[target]=value===2?1:2;analysis=core.analyzeContradiction(regions,n,state,140000,10)}var assumption=value===2?'an X':'a star',result=value===2?'a star':'an X',unitIndex=context.cellUnits[target][0],area=context.units[unitIndex],steps=[
    ...
    ];appendContradictionExplanation(steps,analysis);steps.push({title:'Reverse the impossible assumption',text:'The assumption leads to a rule violation in every legal continuation. Therefore '+squareName(target)+' must be '+result+'.',visual:{area:area,targets:[target],simStars:value===2?[target]:[],simXs:value===1?[target]:[]}});return{target:target,value:value,summary:'Test the opposite mark and trace exactly why every continuation fails.',steps:steps}
  }
